...
describe("DiamondTest", async function () {
  ...
  it("should have 7 facets -- call to facetAddresses function", async () => {
    ...
    assert.equal(addresses.length, 12); // default facets: [diamondCut, diamondLoupe, accessControl, initialization], protocol: [entity, metaTransaction, offer, verification, custody, funds, pause, config, custodyVault]
  });
  ...
  it("facets should have the right function selectors -- call to facetFunctionSelectors function", async () => {
    ...
    assert.sameMembers([...result], selectors);
    selectors = getSelectors(accessControllerFacet).remove(["supportsInterface", "initialize"]);
  ...
  it("selectors should be associated to facets correctly -- multiple calls to facetAddress function", async () => {
    ...
    assert.equal(addresses[2], await diamondLoupeFacet.facetAddress("0xa217fddf"));
  });
  ...
  it("should add test1 functions", async () => {
    ...
    result = await diamondLoupeFacet.facetFunctionSelectors(await test1Facet.getAddress());
    assert.sameMembers([...result], selectors);
  });
  ...
  it("should add test2 functions", async () => {
    ...
    const test2FacetContract = await ethers.getContractAt("Test2Facet", await test2Facet.getAddress());
    addresses.push(await test2Facet.getAddress());
    const selectors = getSelectors(test2FacetContract);
    tx = await diamondCutFacet.diamondCut(
      [
        {
    ...
    result = await diamondLoupeFacet.facetFunctionSelectors(await test2Facet.getAddress());
    assert.sameMembers([...result], selectors);
  });
  ...
  it("add most functions and facets", async () => {
    ...
    const accessControllerFacetSelectors = getSelectors(accessControllerFacet).remove(["supportsInterface(bytes4)"]);
    const Test1Facet = await ethers.getContractFactory("Test1Facet");
    const Test2Facet = await ethers.getContractFactory("Test2Facet");
    ...
    const cut = [
      ...
      },
      {
        ...
        action: FacetCutAction.Add,
        functionSelectors: accessControllerFacetSelectors,
      },
      {
    ...
    assert.sameMembers(
      ...
      accessControllerFacetSelectors,
    );

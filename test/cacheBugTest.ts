...
describe("Cache bug test", async () => {
  ...
  before(async function () {
    ...
    await test1FacetDeploy.waitForDeployment();
    test1Facet = await ethers.getContractAt("Test1Facet", await test1FacetDeploy.getAddress());
  ...
  it("should not exhibit the cache bug", async () => {
    ...
    const selectors = await diamondLoupeFacet.facetFunctionSelectors(await test1Facet.getAddress());

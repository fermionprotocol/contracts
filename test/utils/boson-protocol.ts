...
export async function initBosonProtocolFixture() {
  ...
  const deployedByteCodes = JSON.parse(
    fs.readFileSync("test/utils/boson-protocol-artifacts/deployedByteCodes.json", "utf8"),
  );
  ...
  await provider.send("hardhat_setBalance", [adminAddress, toBeHex(parseUnits("100", "ether"))]);
...
async function setStorage(address: string, pointer: string, value: string) {
  await provider.send("hardhat_setStorageAt", [address, pointer, value]);
}

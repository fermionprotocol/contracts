...
contract FermionWrapper is Ownable, ERC721 {
    ...
    bytes32 private immutable openSeaConduitKey;
    address private immutable openSeaConduit;
    ...
    constructor(
        ...
        // todo: add make correct names + symbol
        ERC721("Fermion F-NFT", "FMION-NFT")
        Ownable(msg.sender)
    {
        // true owner (the seller) is set during the initialization
    ...
    function wrap(uint256[] calldata _tokenIds, address _to) internal {
        ...
        _setApprovalForAll(address(this), openSeaConduit, true); // maybe do it per tokenId?
    }
    ...
    function wrapForAuction(uint256[] calldata _tokenIds, address _to) external {
        wrap(_tokenIds, _to);
    }
    ...
    function getCurrentBalance(uint256 _tokenId) internal returns (address exchangeToken, uint256 balance) {
        ...
        if (exchangeToken == address(0)) {
            ...
            if (offerId == 0) {
                ...
                (, IBosonProtocol.Exchange memory exchange, ) = IBosonProtocol(protocolAddress).getExchange(exchangeId);
            ...
            (, IBosonProtocol.Offer memory offer, , , , ) = IBosonProtocol(protocolAddress).getOffer(offerId);
